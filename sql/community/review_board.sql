...
VALUES 
	(16, 2)
ON DUPLICATE KEY UPDATE following_code = VALUES(following_code);	
...
LEFT JOIN follow f ON p.user_code = f.following_code 
    AND f.following_code = 16  -- 사용자 코드 테스트용
WHERE c.category_name = '후기'
AND p.is_delete = 'N'
...
WHERE c.category_name = '후기'
	 AND p.is_delete = 'N'
    AND p.post_title LIKE CONCAT('%', '집밥', '%')  -- 검색 키워드
...
-- 5. 게시글 추가  
INSERT INTO post (post_title, post_content, user_code, category_id, opu_add_id)
VALUES ('[후기] 건강한 식단 성공 후기', '-5kg 감량 성공', 15, 2, 3);

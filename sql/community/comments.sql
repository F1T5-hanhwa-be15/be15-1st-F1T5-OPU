...
SELECT 
    c.comment_id AS '대댓글 ID',
    c.comment_content AS '대댓글 내용',
    c.create_at AS '생성일',
    COALESCE(cl.댓글좋아요수, 0) AS '댓글 좋아요 수'
FROM comment c
LEFT JOIN (
...
SELECT * FROM comment WHERE user_code = 5;
...
INSERT INTO comment (user_code, comment_content, post_id, reply_comment_id)
VALUES 
    (7, '동의합니다! 좋은 정보네요!', 10, 7);

SELECT * FROM comment WHERE user_code = 7;
...
UPDATE comment
SET is_delete = 'Y'
WHERE comment_id = 7 OR reply_comment_id = 7;
